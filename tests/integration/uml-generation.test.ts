...
The system shall process requests`;
    ...
    try {
      const result = await cli(['generate', testAearsFile]);
      const { stdout } = result;
...
The user shall interact with system`;
    ...
    try {
      const result = await cli(['generate', testAearsFile, '-o', testOutputFile]);
      const { stdout } = result;
...
The system shall not allow unauthorized access`;
    ...
    try {
      const result = await cli(['generate', testAearsFile, '-f', 'report']);
      const { stdout } = result;
...
The system shall respond`;
    ...
    try {
      const result = await cli(['generate', testAearsFile, '--title', '--stats']);
      const { stdout } = result;
...
WHERE security enabled the system shall log activities`;
    ...
    try {
      const result = await cli(['generate', testAearsFile, '--stats']);
      const { stdout } = result;
